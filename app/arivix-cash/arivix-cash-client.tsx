...
import { BankTransferPanel } from "@/components/arivix-cash/bank-transfer-panel";
import { CasinoFloor } from "@/components/arivix-cash/casino-floor";
import { GamesCasinoPanel } from "@/components/arivix-cash/games-casino-panel";
import { RecentWinsTicker } from "@/components/arivix-cash/recent-wins-ticker";
...
export function ArivixCashClient() {
  ...
  const refreshMe = useCallback(async () => {
    setMeError(null);
    const res = await fetch("/api/arivix-cash/me", { cache: "no-store" });
    if (!res.ok) {
  ...
  const refreshLeaderboard = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/arivix-cash/leaderboard?tab=${lbTab}`,
        { cache: "no-store" },
      );
      if (!res.ok) return;
  ...
  async function acceptDisclaimer() {
    ...
    try {
      const res = await fetch("/api/arivix-cash/disclaimer", { method: "POST" });
      if (!res.ok) {
        const j = (await res.json().catch(() => null)) as { error?: string } | null;
