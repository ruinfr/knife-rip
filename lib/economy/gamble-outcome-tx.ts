...
import { GAMBLE_HOUSE_USER_ID } from "./economy-env";
import { creditGambleHouseInTx } from "./wallet-sink-in-tx";
...
export async function applyGambleOutcomeInTx(
  ...
  if (newCash < BigInt(0)) throw new Error("INSUFFICIENT_FUNDS");

  const net = payout - bet;
  const winInc = payout > bet ? 1 : 0;
  const lossInc = payout === BigInt(0) ? 1 : 0;
  const newStreak =
    payout > bet
  ...
  if (net < BigInt(0) && userId !== GAMBLE_HOUSE_USER_ID) {
    await creditGambleHouseInTx(tx, {
      delta: -net,
      actorUserId: userId,
      meta: { game },
    });
