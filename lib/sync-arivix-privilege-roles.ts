...
export type ArivixRipDiscordRoleSyncReport =
 ...
 */
export async function syncArivixRipDiscordRolesForDiscordUser(
  discordUserId: string,
): Promise<ArivixRipDiscordRoleSyncReport> {
  ...
  if (!env || !token) {
    return {
      ...
      detail: !token
        ...
        : "hub guild id not set (ARIVIX_RIP_GUILD_ID or legacy KNIFE_RIP_GUILD_ID; role IDs have defaults)",
    };
  }
  ...
  try {
    ...
    console.error("[arivix-privilege-roles]", discordUserId, e);
 ...
 */
export async function syncArivixRipDiscordRolesForUserId(
  userId: string,
): Promise<ArivixRipDiscordRoleSyncReport | null> {
  const discordId = await resolveDiscordProviderAccountId(userId);
  if (!discordId) return null;
  return syncArivixRipDiscordRolesForDiscordUser(discordId);
}
